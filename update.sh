...
if [ $? -eq 0 ]; then
 systemctl stop piRa1n.service
  if [ -f /home/pi/piRa1n-web/look_for_updates.sh ]; then
    ...
    echo '<br>20%'
    {
      apt-get install git usbmuxd libimobiledevice6 libimobiledevice-utils build-essential checkinstall git autoconf automake libtool-bin libreadline-dev libusb-1.0-0-dev libusbmuxd-tools sshpass -y
        if ! which irecovery >> /dev/null; then
          ...
          git clone https://github.com/libimobiledevice/libirecovery.git 2> /dev/null
          cd libirecovery
    ...
    {
      cd /home/pi/
      git clone https://github.com/raspberryenvoie/piRa1n-web.git temp/ 2> /dev/null
      mv temp/* piRa1n-web/
      rm -rf temp/
    ...
    echo '<br>60%'
    {
      ...
      cd /home/pi/
      git clone https://github.com/raspberryenvoie/piRa1n.git 2> /dev/null
      echo '<br>80%'
      cd piRa1n/
      wget https://assets.checkra.in/downloads/linux/cli/arm/dde0ee4255403a427636bb76e09e409487f8be128af4b7d89fac78548bd5b35a/checkra1n -O checkra1n > /dev/null 2>&1 # Download Checkra1n
      chmod +x checkra1n
